export default function DemoThumbnail({index}:{index:string}){return <div className={`demoThumb demoThumb${index}`} aria-hidden="true">
  {index==="00"&&<div className="thumbAiSuitePreview"><div className="thumbPhone"><header><i/><b>首页　商机　行情</b></header><p><span>AI</span> 螺纹钢行情如何？</p><div className="thumbChart"><i/><i/><i/><i/><i/></div></div><div className="thumbSuitePc"><aside><b>AI</b><i/><i/><i/></aside><main><header>运营总览</header><section><i/><i/><i/></section><div><b/><b/><b/><b/></div></main></div><span>移动用户端　⇄　PC 运营端</span></div>}
