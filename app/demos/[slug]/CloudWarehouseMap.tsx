...
function WmdPublicHeader(){
  ...
  return <header className="wmdPublicHeader">
    ...
    <nav className="wmdNav" aria-label="万贸达公共导航">{wmdNav.map(([icon,label,path],index)=><a className={index===0?"active":""} key={label} href={`${base}${path}`} target="_blank" rel="noreferrer"><i>{icon}</i>{label}</a>)}</nav>
  </header>;
}
