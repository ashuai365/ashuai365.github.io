...
export default function CommodityAiApp(){
  ...
  const [marketView,setMarketView]=useState<MarketView>("价格");
    ...
    <div className="phoneShell">
      ...
      <div className="phoneScreen">
        ...
        {stage==="app"&&<section className="mainScreen">
          ...
          {tab==="行情"&&<div className="marketScreen"><div className="marketTitle"><span>MARKET INTELLIGENCE</span><h2>行情中心</h2><p>覆盖价格、快讯与行业研究</p></div><div className="marketViewTabs">{(["价格","快讯","研报"] as MarketView[]).map(x=><button key={x} className={marketView===x?"active":""} onClick={()=>setMarketView(x)}>{x}</button>)}</div>
            {marketView==="价格"&&<><div className="marketTabs"><button className="active">钢材</button><button>原料</button><button>有色</button></div><div className="marketList">{marketRows.map(row=><button key={row[0]} onClick={()=>{setTab("首页");setChat("market");setStep(1)}}><span><b>{row[0]}</b><small>{row[3]} · 元/吨</small></span><strong>{row[1]}</strong><i className={row[2].startsWith("+")?"rise":"fall"}>{row[2]}</i></button>)}</div><div className="trendCard"><header><span>螺纹钢价格走势</span><div>{["7日","30日","1年"].map(x=><button key={x} className={period===x?"active":""} onClick={()=>setPeriod(x)}>{x}</button>)}</div></header><div className={`trendLine period${period}`}><i/><i/><i/><i/><i/><i/><i/></div><footer><b>最高 3,356</b><b>最新 3,289</b></footer></div></>}
            {marketView==="快讯"&&<div className="flashFeed">{[["10:36","华东建筑钢材成交较早盘回暖，商家报价以稳为主。"],["09:52","主力合约窄幅震荡，市场关注库存去化速度。"],["08:45","多地重大项目集中开工，建材需求预期边际改善。"],["07:30","夜盘铁矿石收涨，钢材成本支撑小幅增强。"]].map((x,i)=><article key={x[0]}><time>{x[0]}</time><div><b>{i===0?"即时":"快讯"}</b><p>{x[1]}</p></div></article>)}</div>}
            {marketView==="研报"&&<div className="reportList">{[["钢材周报","淡季库存拐点何时出现？","供需 · 库存 · 利润"],["产业观察","基建资金到位对建材需求的传导","宏观 · 建筑钢材"],["原料专题","铁矿石估值与钢厂补库节奏","成本 · 原料"]].map((x,i)=><article key={x[1]}><small>{x[0]} · 今日</small><h3>{x[1]}</h3><p>{x[2]}</p><footer><button onClick={()=>{setSaved(true);}}>☆ {saved&&i===0?"已收藏":"收藏"}</button><button onClick={()=>{setTab("首页");setChat("market");setStep(1)}}>AI 解读 →</button></footer></article>)}</div>}
            <small className="demoDataNote">信息结构参考公开行业资讯平台 · 本页均为虚构演示数据</small></div>}
