...
export default function CategoriesPage(){const {page,setPage,totalPages,visibleItems}=usePagination(categories,10);return <div><SiteHeader active="categories"/><main className="hubPage"><header className="hubHero"><span>TOPICS / 内容地图</span><h1>分类</h1><p>沿着几个长期关注的方向，浏览 MADAO 对产品、行业与 AI 的持续思考。</p></header><section className="categoryGrid">{visibleItems.map((item,index)=><a href={item[4] as string} className="categoryTile" key={item[0] as string}><span>{String((page-1)*10+index+1).padStart(2,"0")}</span><small>{item[2]}</small><h2>{item[0]}</h2><p>{item[1]}</p><div>{(item[3] as string[]).map(tag=><i key={tag}># {tag}</i>)}</div><b>阅读代表文章 →</b></a>)}</section><Pagination page={page} totalPages={totalPages} onChange={setPage}/><nav className="channelNext" aria-label="继续浏览"><a href="/">← 返回文章首页</a><a href="/tags/">按标签继续浏览 →</a></nav></main><footer className="siteFooter"><p>© 2026 MADAO · 一个产品经理的网站</p></footer></div>}
