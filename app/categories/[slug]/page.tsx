...
export default async function CategoryContentPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <div><SiteHeader active="categories"/><main className="hubPage contentIndexPage"><header className="contentIndexHero"><a href="/categories/">← 返回全部分类</a><span>CATEGORY / 内容分类</span><h1>{category.name}</h1><p>{category.description}，共 {items.length} 篇内容，按更新时间倒序排列。</p></header><ContentList items={items}/><nav className="channelNext"><a href="/categories/">查看其他分类</a><a href="/tags/">按标签浏览 →</a></nav></main><footer className="siteFooter"><p>© 2026 MADAO · 一个产品经理的网站</p></footer></div>;
}
